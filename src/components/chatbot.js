    ...
    return (
        <div>
            <div className="chat-window" id="chat" style={{ display: 'none' }}>
                ...
                <p className="close-btn" onClick={chatToggle}>X</p>
                ...
                <form id="chat-form">
                    <input
                        ...
                        required
                    />
                    <button type="submit" onClick={chat}>Send</button>
                </form>
            </div>

            <button id="btn" className="chat-btn" onClick={chatToggle}><i class="fa-regular fa-comments"></i></button>
        </div>
    );
}
...
function chat() {
    ...
    const apiKey = "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";
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (message !== "") {
            ...
            if (message.toLowerCase().includes("who are you")) {
                const predefinedResponse = "I am the KiddieEd chatbot. How can I help you today?";
                ...
                const predefinedResponse = "KiddieEd is an online learning platform for kids";
                messages.innerHTML += `<div class="message bot-message"><span>${predefinedResponse}</span>
    </div>`;
            }
            else if (message.toLowerCase().includes("hello")) {
                const predefinedResponse = "Hello,I am the KiddieEd chatbot. How can I help you today?";
                messages.innerHTML += `<div class="message bot-message"><span>${predefinedResponse}</span>
    </div>`;
            }

            else if (message.toLowerCase().includes("what are the courses you have")) {
                ...
                const response = await axios.post("https://api.openai.com/v1/completions", d1, d2);
